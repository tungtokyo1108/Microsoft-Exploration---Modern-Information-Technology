 ...
 * https://github.com/Microsoft/ELL/blob/master/libraries/math/src/BlasWrapper.cpp
 *
...
namespace ell
{
    namespace math
    {
        namespace Blas
        {
            int GetBlasMatrixOrder(MatrixLayout order)
            {
                switch (order)
                {
                    case MatrixLayout::rowMajor:
                        #if USE_BLAS
                            return CBLAS_ORDERS::CblasRowMajor;
                        #else
                            return 101;
                        #endif
            ...
            {
                cblas_sgemv(static_cast<CBLAS_ORDER>(GetCBlasMatrixOrder(order)), static_cast<CBLAS_TRANSPOSE>(transpose),
                                m, n, alpha, M, lda, x, incx, beta, y, incy);
            }
            ...
            {
                cblas_dgemv(static_cast<CBLAS_ORDER>(GetCBlasMatrixOrder(order)), static_cast<CBLAS_TRANSPOSE>(transpose),
                                m, n, alpha, M, lda, x, incx, beta, y, incy);
            }
            ...
            {
                cblas_sgemm(static_cast<CBLAS_ORDER>(GetCBlasMatrixOrder(order)), static_cast<CBLAS_TRANSPOSE>(GetCBlasMatrixTranspose(transposeA)),
                    static_cast<CBLAS_TRANSPOSE>(GetCBlasMatrixTranspose(transposeB)), m, n, k, alpha, A, lda, B, ldb, beta, C, ldc);
            }
            void Gemm(MatrixLayout order, MatrixTranspose transposeA, MatrixTranspose transposeB, int m, int n, int k, 
                        double alpha, const double* A, int lda, const double* B, int ldb, double beta, double* C, int ldc)
            {
                cblas_dgemm(static_cast<CBLAS_ORDER>(GetCBlasMatrixOrder(order)), static_cast<CBLAS_TRANSPOSE>(GetCBlasMatrixTranspose(transposeA)),
                    static_cast<CBLAS_TRANSPOSE>(GetCBlasMatrixTranspose(transposeB)), m, n, k, alpha, A, lda, B, ldb, beta, C, ldc);
            }
#endif
        }
