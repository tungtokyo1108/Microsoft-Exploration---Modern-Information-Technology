 ...
 * https://github.com/microsoft/ELL/blob/master/libraries/math/include/MatrixOperations.h
 *
...
namespace ell 
{
    namespace math
    {
        ...
        void Print(ConstMatrixReference<ElementType, layout> M, std::ostream& stream, size_t indent = 0, 
                    size_t maxRows = std::numeric_limits<size_t>::max(), size_t maxElementsPerRow = std::numeric_limits<size_t>::max());
        
        // Prints a matrix in initilizer list format
        template <typename ElementType, MatrixLayout layout>
...
// #include <ellutilities/include/Exception.h>
...
namespace ell
{
    namespace math
    {
        ...
        void Print(ConstMatrixReference<ElementType, layout> M, std::ostream& stream, size_t indent, size_t maxRows, size_t maxElementsPerRow)
        {
            ...
            if (M.NumRows() <= maxElementsPerRow)
            {
            ...
            else
            {
                ...
                stream << "," << EOL << std::string(indent + 2, ' ') << "...," << EOL;
